...
- `(cd src-tauri && cargo audit --json > /tmp/communityglows-cargo-audit-before.json)` - pre-migration baseline.
...
- `(cd src-tauri && cargo audit --json > /tmp/communityglows-cargo-audit-accepted.json)` - post-migration RustSec posture.
