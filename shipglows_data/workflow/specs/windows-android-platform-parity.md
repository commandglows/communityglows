...
linked_systems:
  ...
  - "shipglows_data/workflow/tauri-mobile.md"
  - "shipglows_data/workflow/audits/2026-08-02-platform-parity-windows-android.md"
...
depends_on:
  ...
  - artifact: "shipglows_data/workflow/audits/2026-08-02-platform-parity-windows-android.md"
...
- `shipglows_data/workflow/test-checklists/windows-android-platform-parity.md`
...
- `shipglows_data/technical/context.md`: native host and parity contract.
- `shipglows_data/technical/android-webview-session-isolation.md`: align Windows comparison and fallback boundaries.
- `shipglows_data/workflow/tauri-mobile.md`: Windows/Android artifact and QA instructions.
- `shipglows_data/workflow/test-checklists/windows-android-platform-parity.md`: executed manual proof.
- `shipglows_data/workflow/audits/2026-08-02-platform-parity-windows-android.md`: update verdict cells only after evidence.
...
- [ ] Task 8: Execute platform QA and update claims.
  - Files: `shipglows_data/workflow/test-checklists/windows-android-platform-parity.md`, `README.md`, `shipglows_data/technical/context.md`, `shipglows_data/workflow/tauri-mobile.md`, audit report.
...
| 2026-08-08 UTC | 001-sg-build | GPT-5 Codex | Reused `ProfileSwitcher` in the Windows right panel so profile switching and management follow the same dropdown flow as the left panel, and removed the permanent profile-management button; typecheck/core, 127 tests, and zero token drift pass locally. | implemented-pending-windows-proof | Confirm both panel dropdowns and the shared manager in the fresh installed-Windows walkthrough before closing Task 3. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Moved the hidden-panel control bar inside the central desktop column so an open left or right sidebar keeps its full height while top and bottom bars stop flush against it; typecheck/core and changed-file token drift checks pass. | implemented-pending-visual-proof | Verify the four left/right and top/bottom resize combinations in the Windows shell. |
| 2026-08-08 UTC | 003-sg-bug / 106-sg-fix | GPT-5 Codex | Connected the Windows settings sheet to the canonical desktop and ultrawide breakpoints, grouping account, service, and preference controls into responsive two- and three-column layouts; typecheck/core and changed-file token drift checks pass. | fix-attempted | Confirm the settings layout at narrow, 1280 px, and 1600 px or wider in the Windows shell. |
| 2026-08-08 UTC | 001-sg-build | GPT-5 Codex | Extracted the settings drawer into a shared accessible `SgSheet` and migrated profile management from its popup to the same responsive, tactile surface; profile management uses one column when narrow and two columns at the canonical desktop breakpoint, with one shared scroll owner. | implemented-pending-windows-proof | Verify settings and profile sheets at narrow and desktop widths, including focus, Escape, overlay close, drag dismissal, and profile save/delete flows. |
| 2026-08-08 UTC | 001-sg-build | GPT-5 Codex | Added one shared profile-avatar editor and a single app-level picker opened from Settings and the right-panel avatar pencil; photo upload/removal and emoji selection now share validation with profile management and save through one atomic profile update. | implemented-pending-windows-proof | Verify file selection, emoji selection, cancel, photo removal, and synchronized avatar rendering in both desktop panels on Windows. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Replaced the profile manager's visible-network checkbox list with responsive selectable cards using canonical network icons, selected borders, check indicators, and native checkbox semantics for keyboard and assistive technology. | implemented-pending-visual-proof | Verify card selection, focus visibility, long labels, and narrow/desktop wrapping in the Windows profile drawer. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Removed hover/focus information bubbles across the CommunityGlows app by disabling the shared tooltip directive, removing native button titles, and preserving accessible names for icon-only controls. | implemented-pending-visual-proof | Confirm no tooltip appears on mouse hover or keyboard focus while icon controls retain clear accessible labels. |
| 2026-08-08 UTC | 003-sg-bug / 106-sg-fix | GPT-5 Codex | Prevented the left profile switcher footer from shrinking out of view when the right splitter is removed by giving the shared switcher a full-width, non-shrinking flex contract independent of right-panel state. | fix-attempted | Close and reopen the right panel with top and bottom control bars and confirm the left profile switcher remains visible. |
| 2026-08-08 UTC | 001-sg-build | GPT-5 Codex | Added a compact shared profile selector and the active profile's visible network icons to the desktop horizontal control bar when both sidebars are closed; the profile menu opens inward from either top or bottom position and the network row remains horizontally usable at narrow widths. | implemented-pending-windows-proof | Verify profile switching, network navigation, overflow, and both top/bottom bar positions in the Windows shell. |
| 2026-08-08 UTC | 104-sg-end | GPT-5 Codex | Closed the local implementation pass for the Windows profile and panel UI with typecheck, focused profile-store tests, changed-file design drift, and diff checks passing; documentation reflection is captured in this spec and the changelog, while the canonical architecture and stores remain unchanged. | partial/deferred-windows-proof | Keep AC13-AC20 and the panel, sheet, avatar, tooltip, scrollbar, and collapsed-navigation walkthroughs open until a fresh installed-Windows session provides visual and functional proof. |
| 2026-08-08 UTC | 005-sg-ship | GPT-5 Codex | Shipped the bounded source commit for the unified Windows profile-management and collapsed-navigation UI after local typecheck, focused tests, Tauri frontend build, token, drift, and diff checks passed; no Windows installer or visual proof is claimed. | shipped-pending-windows-proof | Execute the fresh installed-Windows walkthrough for profile, panel, sheet, avatar, scrollbar, and top/bottom collapsed-navigation behavior. |
| 2026-08-08 UTC | 003-sg-bug / 106-sg-fix | GPT-5 Codex | Restored centering of the main Get Started/sign-in card by giving the login surface the full central-column width after that column became a flex container; typecheck, Tauri frontend build, design drift, and diff checks pass. | fix-attempted | Confirm the card remains centered with both panels open, one panel open, and both panels collapsed on Windows. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Simplified the right-panel profile heading by hiding its redundant compact emoji and centering the active profile name beneath the large avatar, while preserving the shared switcher behavior elsewhere. Reversed both sidebar resize-handle hover and focus colors through the existing theme-aware on-action token so they resolve light in light mode and dark in dark mode with less glare; typecheck, Tauri frontend build, token validation/check, design drift, and diff checks pass. | fix-attempted | Confirm the right profile-name alignment and both resize handles in light and dark themes on an installed Windows build. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Reorganized the profile manager's identity section into a compact responsive surface: the name and shared avatar editor stack at narrow widths and use a balanced two-column composition at the canonical desktop breakpoint, while the visible-network cards and all profile actions remain unchanged. | fix-attempted | Confirm the identity layout, controls, and wrapping at narrow and desktop drawer widths in an installed Windows build. |
| 2026-08-08 UTC | 003-sg-bug / 106-sg-fix | GPT-5 Codex | Removed the collapsed desktop network row's permissive no-profile fallback, derived its dense list directly from the active profile's reactive hidden-network set, and added a click-time visibility guard so a hidden network cannot be opened from a stale render. | fix-attempted | On installed Windows, hide and restore networks while both panels are collapsed and confirm the top and bottom bars update immediately without gaps or activatable hidden entries. |
| 2026-08-08 UTC | 001-sg-build | GPT-5 Codex | Kept both desktop splitter trees and their central slots mounted while either panel is hidden, collapsed the hidden panel's Reka allocation to zero, and restored its last visible size on reopening, so panel/bar moves only reflow the stable WebView host and send native bounds updates instead of closing and reopening WebView2. | implemented-pending-windows-proof | On installed Windows, move the bar top/bottom and close/reopen both panels while a network is active; confirm the center fills the available width and session, scroll position, media state, and rendering remain continuous without a flash. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Made the desktop horizontal bar resizable only while both sidebars are collapsed. Its persisted height is clamped to 20% of the current viewport, its top/bottom handle grows inward with matching pointer and keyboard semantics, and the shared profile selector plus visible-network icons scale from canonical tokens without remounting the central WebView host. | implemented-pending-visual-proof | On installed Windows, collapse both panels, resize top and bottom bars with pointer and keyboard in light/dark modes, relaunch to confirm persistence, and confirm the active WebView keeps its session and rendering continuity. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Capped each desktop sidebar at 30% of its Reka splitter container through the official `maxSize` constraint, while retaining the 5% compact minimum and zero-size collapsed state. Restored sizes are clamped to the same shared range, so an older oversized value cannot re-expand beyond the new cap; this is a layout-only constraint and does not touch the native WebView lifecycle. | implemented-pending-visual-proof | On installed Windows, resize both sidebars separately and together, then collapse/reopen each at narrow and wide desktop widths; confirm the 30% cap, compact mode, 0% collapse, and central WebView continuity. |
| 2026-08-08 UTC | 003-sg-bug / 106-sg-fix | GPT-5 Codex | Replaced the shared profile-switcher trigger's simulated `div` button with one native full-width button, so the active profile name, identity, and chevron are one explicit, keyboard-accessible control in the collapsed desktop bar and both sidebars. Its popup state is exposed through `aria-expanded`; the resize handle remains confined to the bar's inner edge. Typecheck, focused bar-store tests, Tauri frontend build, changed-file drift, and diff checks pass locally. | fix-attempted | On installed Windows, collapse both panels and click the active profile name, emoji, and chevron in both top and bottom bars; confirm each opens the same selector, retains focus behavior, and does not interfere with resizing. |
| 2026-08-08 UTC | 006-sg-design | GPT-5 Codex | Bound the sidebar collapse controls to the persisted control-bar position: they remain in their current top corners for a top bar, and move to dedicated bottom-left/bottom-right footer zones for a bottom bar. The left footer preserves the profile selector immediately above it; the right footer stays reachable when its content scrolls. | implemented-pending-visual-proof | On installed Windows, switch the bar between top and bottom with each sidebar expanded and compact; confirm the two controls stay in their respective corners, the left selector remains reachable, and keyboard focus/collapse still work. |
| 2026-08-09 UTC | 006-sg-design | GPT-5 Codex | Moved the collapsed-bar profile selector into a dedicated leading slot immediately after the left-panel reopen control, while keeping the visible-network icon row centered. It mounts only when both desktop sidebars are closed; every panel-open layout and its existing profile selector remain unchanged. | implemented-pending-visual-proof | On installed Windows, close both panels and confirm the profile trigger opens beside the left reopen button in both top and bottom bars, network icons stay centered and scroll on overflow, and reopening either panel restores its existing selector without duplication. |
| 2026-08-09 UTC | 003-sg-bug / 106-sg-fix | GPT-5 Codex | Corrected the collapsed-bar visibility contract at its data source: legacy and local placeholder profiles without a persisted `hiddenNetworks` selection now resolve to the catalogue defaults instead of exposing the entire catalogue. The desktop bar, profile manager, extension parity surface, store guards, cloud normalization, and visibility toggles share the same resolver; hidden entries are omitted from the dense array and cannot reserve a slot or be activated. Focused visibility/profile regressions (15 tests), core typecheck, Tauri frontend build, and diff checks pass locally; changed-file drift reports only the catalogue's 56 existing brand-color literals. | fix-attempted | On installed Windows, use a pre-existing profile and a newly created profile, disable networks, collapse both panels, and confirm only selected networks remain in a gap-free row and hidden networks cannot be activated in both top and bottom positions. |
...
- Task 3 (desktop profile manager): implemented-pending-windows-proof — the single manager, atomic create/edit/delete flow, submit lock, validation, shared-state behavior, accessible errors, and responsive shared-sheet presentation are implemented and locally verified; AC13-AC20 remain open pending the fresh installed-Windows walkthrough.
...
- `006-sg-design`: implemented-pending-visual-proof — token consumption, central-column control-bar layout, 30%-bounded sidebar resize, persisted 20%-bounded compact-bar resize, position-aware sidebar collapse controls, and a both-panels-closed leading profile selector are complete; panel and bar combinations still need installed Windows visual proof.
- `003-sg-bug / 106-sg-fix`: fix-attempted — collapsed navigation now resolves legacy/default profile visibility through the same canonical selection contract as profile management, renders only a dense visible-network array, rejects stale hidden-network clicks, and exposes its full profile trigger as one native accessible button; top/bottom installed-Windows proof is still required.
- `001-sg-build`: implemented-pending-windows-proof — desktop panel visibility now preserves the splitter trees and central WebView/router component identity; only layout bounds change, with flash-free continuity still requiring an installed-Windows walkthrough.
- `104-sg-end`: partial/deferred-windows-proof — local implementation and proportional automated checks are closed; installed-Windows visual and functional proof remains required before full closure.
- `005-sg-ship`: shipped-pending-windows-proof — bounded source commit shipped; fresh installed-Windows visual and functional proof remains pending.
