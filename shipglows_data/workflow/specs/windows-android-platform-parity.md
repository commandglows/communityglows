...
- `006-sg-design`: implemented-pending-visual-proof — token consumption, central-column control-bar layout, 30%-bounded sidebar resize, persisted 20%-bounded compact-bar resize, and position-aware sidebar collapse controls are complete; panel and bar combinations still need installed Windows visual proof.
- `003-sg-bug / 106-sg-fix`: fix-attempted — collapsed navigation now renders only the active profile's visible networks, rejects stale clicks, and exposes its full profile trigger (name, identity, and chevron) as one native accessible button; top/bottom bar interaction and resizing still need installed-Windows proof.
