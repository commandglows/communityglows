...
- `006-sg-design`: implemented-pending-visual-proof — token consumption and the central-column control-bar layout are complete; the left/right × top/bottom resize combinations still need installed Windows visual proof.
- `003-sg-bug / 106-sg-fix`: fix-attempted — collapsed navigation now renders only the active profile's visible networks and rejects stale clicks; immediate dense updates in both bar positions still need installed-Windows proof.
