...
### Changed
- **Project governance now has one canonical corpus** — migrated active documentation, specs, trackers, audits, and useful archives to root `shipglows_data/`; removed legacy root and nested site governance copies after preservation proof.
- **Governance entrypoints now resolve canonical paths** — updated repository and site agent guidance, internal links, and metadata references to the canonical corpus.

### Added
- **Migration preservation is auditable** — archived all nine divergent site governance files byte-for-byte and added a preservation ledger covering every migrated root surface.
