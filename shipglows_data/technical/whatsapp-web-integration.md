...
- **Reverse-engineer the Noise protocol directly** — possible (see `whatsmeow`, `Baileys`) but means shipping a full WA client, not a WebView. Out of scope for CommunityGlows.
...
2. `src/ui/setup/pages/CommunityGlows/components/AppSidebar.vue` — menu item id 13
3. `src/ui/setup/pages/CommunityGlows/components/MobileLayout.vue` — menu item id 13
4. `src/ui/setup/pages/CommunityGlows/components/OnboardingFlow.vue` — `NETWORKS` array
5. `src-tauri/plugins/android-webview/android/src/main/java/com/communityglows/webview/NativeWebViewPlugin.kt` — `NetworkInfo("whatsapp", ...)` in the NETWORKS list (around line 152)
