...
const DEFAULT_PRODUCT_ID = "communityglows";
...
async function runCli() {
  ...
  const program = new Command()
    .name("importCommunityGlowsActivationCodes")
    .description("Import CommunityGlows Lifetime Deal or early-bird activation codes through the suite entitlement bridge.")
    ...
    .option("--admin-secret <secret>", "Admin secret; defaults to COMMUNITYGLOWS_BILLING_ADMIN_SECRET")
  ...
  const adminSecret = options.adminSecret ?? process.env.COMMUNITYGLOWS_BILLING_ADMIN_SECRET;
  if (!adminSecret) throw new Error("missing_communityglows_billing_admin_secret");
